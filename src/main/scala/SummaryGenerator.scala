...
object SummaryGenerator {
  ...
  def generateSummaryTable(
    ...
    // ---------------------------
    // Normalize duplicates input
    // origin -> "reference" | "new" (acepta variantes en ES/EN)
    // exact_duplicates, duplicates_w_variations, occurrences -> long
    // ---------------------------
    val dupNorm = dupDf
    ...
    // ---------------------------
    // Exact Duplicates (IDs con exactos y SIN variaciones)
    // -> cuentan por ID (no por filas)
    // ---------------------------
    val exactDupIdsRef = dupNorm
      .filter(col("origin_norm") === "reference" && col("exact_l") > 0 && col("var_l") === 0)
      .select(col("id_str").as("id")).distinct()

    val exactDupIdsNew = dupNorm
      .filter(col("origin_norm") === "new" && col("exact_l") > 0 && col("var_l") === 0)
      .select(col("id_str").as("id")).distinct()

    val exactDupCountRef = exactDupIdsRef.count()
    val exactDupCountNew = exactDupIdsNew.count()
    val exactDupExamples = exactDupIdsRef.union(exactDupIdsNew).distinct()
      .selectExpr("CAST(id AS STRING)").as[String].collect().mkString(",")

    // ---------------------------
    // Duplicates with variations (IDs)
    // ---------------------------
    val variedDupIdsRef = dupNorm
      .filter(col("origin_norm") === "reference" && col("var_l") > 0)
      .select(col("id_str").as("id")).distinct()

    val variedDupIdsNew = dupNorm
      .filter(col("origin_norm") === "new" && col("var_l") > 0)
      .select(col("id_str").as("id")).distinct()

    // ---------------------------
    // Exact 1:1 Matches (IDs)
    // Intersect full rows y dedupe por id
    // ---------------------------
    ...
    // ---------------------------
    val summaryData = Seq(
      ("Exact Duplicates",                 exactDupCountRef.toString,             exactDupCountNew.toString,        pct(exactDupCountRef), "Exact",      exactDupExamples),
